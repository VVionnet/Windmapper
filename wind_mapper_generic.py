...
def main():
	...
	if len(sys.argv) == 1:
		print('ERROR: wind_mapper.py requires one argument [configuration file] (i.e. wind_mapper.py param_existing_DEM.py)')
		return
	...
	if(use_existing_dem):
		...
		elevation.clip(bounds=(lon_min-delta_lon*fac, lat_min-delta_lat*fac, lon_max+delta_lon*fac, lat_max+delta_lat*fac), output=fic_download)
	...
	for i in range(0,nopt_x):
		for j in range(0,nopt_y):
			...
			for wdir in np.arange(0,360., delta_wind):
				...
				os.system('./WindNinja_cli '+fic_config_WN+' --elevation_file '+fic_dem_in+' --mesh_resolution '+str(res_wind)+' --input_direction '+str(wdir)+' --output_path '+dir_tmp)
	...
	for wdir in np.arange(0,360., delta_wind):
		for var in list_tif_2_vrt:
			...
			cmd = "find "+user_output_dir+" -type f -name '*_"+str(int(wdir))+"_10_"+str(res_wind)+"m_"+var+"*.tif' -exec gdalbuildvrt "+name_vrt+" {} +"
			os.system(cmd)


def clip_tif(fic_in, fic_out,xmin,xmax,ymin,ymax):
     
    com_string = "gdal_translate -of GTIFF -projwin " + str(xmin)+ ", " + str(ymax) + ", " + str(xmax) + ", " + str(ymin) + " " + fic_in + " " + fic_out
